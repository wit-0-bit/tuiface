...
static void draw_aqi_uv_complication(GContext *ctx, GRect box_rect) {
  ...
  graphics_draw_text(ctx, aqi_str, font, aqi_rect, GTextOverflowModeWordWrap, GTextAlignmentRight, NULL);
  
  // Draw Separator (centered)
  graphics_context_set_text_color(ctx, s_active_theme->text_primary);
  graphics_draw_text(ctx, " / ", font, GRect(box_rect.origin.x, y_offset, box_rect.size.w, 20), GTextOverflowModeWordWrap, GTextAlignmentCenter, NULL);
  ...
  graphics_draw_text(ctx, uv_str, font, uv_rect, GTextOverflowModeWordWrap, GTextAlignmentLeft, NULL);
}
